...
func parsePackage(data string) (int, time.Duration, error) {
	...
	if strings.Contains(parts[0], " ") || strings.Contains(parts[1], " ") {
		return 0, 0, errors.New("ввод содержит пробелы")
	}
...
func DayActionInfo(data string, weight, height float64) string {
	...
	if err != nil {
		log.Printf("Ошибка в парсинге строки: %v", err)
		return ""
	}

	if step <= 0 {
		log.Printf("Количество шагов меньше 0: %d", step)
		return ""
	}
	...
	if err != nil {
		log.Printf("Ошибка в расчете калорий: %v", err)
		return ""
	}
